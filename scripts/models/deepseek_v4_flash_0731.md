...
- **模型路径**: `/nfs/ofs-luban-data/model/deepseek-ai/DeepSeek-V4-Flash-0731`

# 模型配置

<details><summary>原始 config.json</summary>

`/nfs/ofs-luban-data/model/deepseek-ai/DeepSeek-V4-Flash-0731/config.json`
...
| 权重名称 | 形状 | 数据类型 | 大小 | 文件 |
| --- | --- | --- | --- | --- |
| `embed.weight` | `[129280, 4096]` | `torch.bfloat16` | 1010.00 MB | model-00001-of-00048.safetensors |
| `hc_head_base` | `[4]` | `torch.float32` | 16.00 B | model-00045-of-00048.safetensors |
| `hc_head_fn` | `[4, 16384]` | `torch.float32` | 256.00 KB | model-00045-of-00048.safetensors |
| `hc_head_scale` | `[1]` | `torch.float32` | 4.00 B | model-00045-of-00048.safetensors |
| `head.weight` | `[129280, 4096]` | `torch.bfloat16` | 1010.00 MB | model-00045-of-00048.safetensors |
...
| `mtp.0-2.attn.attn_sink` (×3 mtp) | `[64]` | `torch.float32` | 768.00 B | Multi Files |
| `mtp.0-2.attn.kv_norm.weight` (×3 mtp) | `[512]` | `torch.bfloat16` | 3.00 KB | Multi Files |
| `mtp.0-2.attn.q_norm.weight` (×3 mtp) | `[1024]` | `torch.bfloat16` | 6.00 KB | Multi Files |
| `mtp.0-2.attn.wkv.scale` (×3 mtp) | `[4, 32]` | `torch.float8_e8m0fnu` | 384.00 B | Multi Files |
| `mtp.0-2.attn.wkv.weight` (×3 mtp) | `[512, 4096]` | `torch.float8_e4m3fn` | 6.00 MB | Multi Files |
| `mtp.0-2.attn.wo_a.scale` (×3 mtp) | `[64, 32]` | `torch.float8_e8m0fnu` | 6.00 KB | Multi Files |
| `mtp.0-2.attn.wo_a.weight` (×3 mtp) | `[8192, 4096]` | `torch.float8_e4m3fn` | 96.00 MB | Multi Files |
| `mtp.0-2.attn.wo_b.scale` (×3 mtp) | `[32, 64]` | `torch.float8_e8m0fnu` | 6.00 KB | Multi Files |
| `mtp.0-2.attn.wo_b.weight` (×3 mtp) | `[4096, 8192]` | `torch.float8_e4m3fn` | 96.00 MB | Multi Files |
| `mtp.0-2.attn.wq_a.scale` (×3 mtp) | `[8, 32]` | `torch.float8_e8m0fnu` | 768.00 B | Multi Files |
| `mtp.0-2.attn.wq_a.weight` (×3 mtp) | `[1024, 4096]` | `torch.float8_e4m3fn` | 12.00 MB | Multi Files |
| `mtp.0-2.attn.wq_b.scale` (×3 mtp) | `[256, 8]` | `torch.float8_e8m0fnu` | 6.00 KB | Multi Files |
| `mtp.0-2.attn.wq_b.weight` (×3 mtp) | `[32768, 1024]` | `torch.float8_e4m3fn` | 96.00 MB | Multi Files |
| `mtp.0-2.attn_norm.weight` (×3 mtp) | `[4096]` | `torch.bfloat16` | 24.00 KB | Multi Files |
| `mtp.0-2.ffn.experts.0-255.w1.scale` (×3 mtp, ×256 experts) | `[2048, 128]` | `torch.float8_e8m0fnu` | 192.00 MB | Multi Files |
| `mtp.0-2.ffn.experts.0-255.w1.weight` (×3 mtp, ×256 experts) | `[2048, 2048]` | `torch.int8` | 3.00 GB | Multi Files |
| `mtp.0-2.ffn.experts.0-255.w2.scale` (×3 mtp, ×256 experts) | `[4096, 64]` | `torch.float8_e8m0fnu` | 192.00 MB | Multi Files |
| `mtp.0-2.ffn.experts.0-255.w2.weight` (×3 mtp, ×256 experts) | `[4096, 1024]` | `torch.int8` | 3.00 GB | Multi Files |
| `mtp.0-2.ffn.experts.0-255.w3.scale` (×3 mtp, ×256 experts) | `[2048, 128]` | `torch.float8_e8m0fnu` | 192.00 MB | Multi Files |
| `mtp.0-2.ffn.experts.0-255.w3.weight` (×3 mtp, ×256 experts) | `[2048, 2048]` | `torch.int8` | 3.00 GB | Multi Files |
| `mtp.0-2.ffn.gate.bias` (×3 mtp) | `[256]` | `torch.float32` | 3.00 KB | Multi Files |
| `mtp.0-2.ffn.gate.weight` (×3 mtp) | `[256, 4096]` | `torch.bfloat16` | 6.00 MB | Multi Files |
| `mtp.0-2.ffn.shared_experts.w1.scale` (×3 mtp) | `[16, 32]` | `torch.float8_e8m0fnu` | 1.50 KB | Multi Files |
| `mtp.0-2.ffn.shared_experts.w1.weight` (×3 mtp) | `[2048, 4096]` | `torch.float8_e4m3fn` | 24.00 MB | Multi Files |
| `mtp.0-2.ffn.shared_experts.w2.scale` (×3 mtp) | `[32, 16]` | `torch.float8_e8m0fnu` | 1.50 KB | Multi Files |
| `mtp.0-2.ffn.shared_experts.w2.weight` (×3 mtp) | `[4096, 2048]` | `torch.float8_e4m3fn` | 24.00 MB | Multi Files |
| `mtp.0-2.ffn.shared_experts.w3.scale` (×3 mtp) | `[16, 32]` | `torch.float8_e8m0fnu` | 1.50 KB | Multi Files |
| `mtp.0-2.ffn.shared_experts.w3.weight` (×3 mtp) | `[2048, 4096]` | `torch.float8_e4m3fn` | 24.00 MB | Multi Files |
| `mtp.0-2.ffn_norm.weight` (×3 mtp) | `[4096]` | `torch.bfloat16` | 24.00 KB | Multi Files |
| `mtp.0-2.hc_attn_base` (×3 mtp) | `[24]` | `torch.float32` | 288.00 B | Multi Files |
| `mtp.0-2.hc_attn_fn` (×3 mtp) | `[24, 16384]` | `torch.float32` | 4.50 MB | Multi Files |
| `mtp.0-2.hc_attn_scale` (×3 mtp) | `[3]` | `torch.float32` | 36.00 B | Multi Files |
| `mtp.0-2.hc_ffn_base` (×3 mtp) | `[24]` | `torch.float32` | 288.00 B | Multi Files |
| `mtp.0-2.hc_ffn_fn` (×3 mtp) | `[24, 16384]` | `torch.float32` | 4.50 MB | Multi Files |
| `mtp.0-2.hc_ffn_scale` (×3 mtp) | `[3]` | `torch.float32` | 36.00 B | Multi Files |
| `mtp.0.main_norm.weight` | `[4096]` | `torch.bfloat16` | 8.00 KB | model-00046-of-00048.safetensors |
| `mtp.0.main_proj.scale` | `[32, 96]` | `torch.float8_e8m0fnu` | 3.00 KB | model-00046-of-00048.safetensors |
| `mtp.0.main_proj.weight` | `[4096, 12288]` | `torch.float8_e4m3fn` | 48.00 MB | model-00046-of-00048.safetensors |
| `mtp.2.confidence_head.proj.weight` | `[1, 4352]` | `torch.bfloat16` | 8.50 KB | model-00048-of-00048.safetensors |
| `mtp.2.hc_head_base` | `[4]` | `torch.float32` | 16.00 B | model-00048-of-00048.safetensors |
| `mtp.2.hc_head_fn` | `[4, 16384]` | `torch.float32` | 256.00 KB | model-00048-of-00048.safetensors |
| `mtp.2.hc_head_scale` | `[1]` | `torch.float32` | 4.00 B | model-00048-of-00048.safetensors |
| `mtp.2.markov_head.markov_w1.weight` | `[129280, 256]` | `torch.bfloat16` | 63.12 MB | model-00048-of-00048.safetensors |
| `mtp.2.markov_head.markov_w2.weight` | `[129280, 256]` | `torch.bfloat16` | 63.12 MB | model-00048-of-00048.safetensors |
| `mtp.2.norm.weight` | `[4096]` | `torch.bfloat16` | 8.00 KB | model-00048-of-00048.safetensors |
| `norm.weight` | `[4096]` | `torch.bfloat16` | 8.00 KB | model-00045-of-00048.safetensors |
