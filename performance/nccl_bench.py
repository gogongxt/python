...
class NcclBenchmark:
    ...
    def init_distributed(self):
        ...
        if "RANK" not in os.environ or "WORLD_SIZE" not in os.environ:
            raise RuntimeError(
                "Run with torchrun: torchrun --nproc_per_node=N nccl.py"
            )
    ...
    def _time_op(self, op_func: Callable, num_warmup: int, num_iterations: int) -> float:
    ...
    def _make_result(self, op_name: str, data_size_bytes: int,
                     avg_time_ms: float, num_warmup: int, num_iterations: int) -> BenchmarkResult:
    ...
    def bench_allreduce(self, count: int, dtype: torch.dtype,
                        num_warmup: int, num_iters: int) -> BenchmarkResult:
        """data_size = send buffer = count * itemsize"""
        t = torch.randn(count, dtype=dtype, device=self.device)
        avg_ms = self._time_op(lambda: dist.all_reduce(t, op=dist.ReduceOp.SUM),
                               num_warmup, num_iters)
        return self._make_result("AllReduce", t.nbytes, avg_ms, num_warmup, num_iters)

    def bench_allgather(self, count: int, dtype: torch.dtype,
                        num_warmup: int, num_iters: int) -> BenchmarkResult:
        ...
        avg_ms = self._time_op(lambda: dist.all_gather_into_tensor(out, inp),
                               num_warmup, num_iters)
        return self._make_result("AllGather", out.nbytes, avg_ms, num_warmup, num_iters)

    def bench_reducescatter(self, count: int, dtype: torch.dtype,
                            num_warmup: int, num_iters: int) -> BenchmarkResult:
        ...
        avg_ms = self._time_op(lambda: dist.reduce_scatter_tensor(out, inp),
                               num_warmup, num_iters)
        return self._make_result("ReduceScatter", inp.nbytes, avg_ms, num_warmup, num_iters)

    def bench_broadcast(self, count: int, dtype: torch.dtype,
                        num_warmup: int, num_iters: int) -> BenchmarkResult:
        """data_size = message size = count * itemsize"""
        t = torch.randn(count, dtype=dtype, device=self.device)
        avg_ms = self._time_op(lambda: dist.broadcast(t, src=0),
                               num_warmup, num_iters)
        return self._make_result("Broadcast", t.nbytes, avg_ms, num_warmup, num_iters)

    def bench_alltoall(self, count: int, dtype: torch.dtype,
                       num_warmup: int, num_iters: int) -> BenchmarkResult:
        ...
        avg_ms = self._time_op(lambda: dist.all_to_all_single(out, inp),
                               num_warmup, num_iters)
    ...
    def run(
        ...
        log("\n" + "=" * 95)
        log(f"  NCCL Benchmark  |  world_size={self.world_size}  |  dtype={dtype}  |  "
            f"warmup={num_warmup}  iters={num_iters}")
        log("=" * 95)
        log(f"{'Op':15s} | {'DataSize(MB)':>12s} | {'Time(ms)':>10s} | "
        ...
        for count in counts:
            for op_name, bench_fn in op_registry:
                ...
                log(f"{r.op_name:15s} | {r.data_size_bytes / 1024**2:12.2f} | "
...
def main():
    parser = argparse.ArgumentParser(description="NCCL benchmark (nccl-tests conventions)")
